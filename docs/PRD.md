...
**Current Stack:** Next.js App Router, Tailwind CSS, SQLite/Postgres via Prisma ORM, MacroDroid/Android automation, Telegram Bot API.
...
Note: Prisma SQLite connector တွင် native enum မထောက်ပံ့သဖြင့် `type` နှင့် `status` ကို String အဖြစ်သိမ်းပြီး API layer မှ value များကိုထိန်းထားသည်။
...
- `npm install`
- `npx prisma generate`
- Local SQLite table creation
- `npm run build`
- Smoke-tested customer creation, KPay webhook parsing, and KPay matching flow

Known local note:

- On this machine, `prisma migrate dev` failed with a Prisma SQLite schema engine error. The SQL migration fallback works and is documented in `README.md`.
...
4. Add deployment target, likely Vercel plus managed Postgres for production.
